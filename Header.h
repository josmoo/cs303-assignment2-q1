...
/**
* Asks the user through console output for an integer, stores it through console input, then returns
* 
* @returns					the integer specified by user
* @throws runtime_error		unhandled, throws if user doesn't input an integer
*/
int getUserInt();

/**
* Asks user for an index and a value to replace at that index
* 
* @returns				 the old value, or -1 if invalid index was passed
* @param int*& intarr		the pointer to the dynamic array
* @param int& arrsize		size of the array
* @param int& newValue		value to put in new array, which will be updated by user input
*/
int userReplaceValue(int*& arr, const int& arrsize, int& newValue);


/**
* Asks user for an integer to append to the passed array
* 
* @param int intarr[]		array of ints to modify
* @param int arrsize			size of array
*/
